...
const prioritizeUsers = (users: ChatUser[]) => {
  const aiUsers = users.filter((user) => user.isAiAssistant);
  const otherUsers = users.filter((user) => !user.isAiAssistant);
  return [...aiUsers, ...otherUsers];
};
...
export const useChatStore = create((set, get) => ({
  ...
  getUsers: async () => {
    try {
      ...
      set({ users: prioritizeUsers(data) });
    } catch (error) {
  ...
  subscribeToMessages: () => {
    const { selectedUser } = get() as { selectedUser: ChatUser | null };
    ...
    const setupMessageListener = () => {
      ...
      socket.on("newMessage", (message: Message) => {
        ...
        } = get() as { messages: Message[]; selectedUser: ChatUser | null };
        const currentAuthUser = (useAuth.getState() as { authUser?: any })
          .authUser;
